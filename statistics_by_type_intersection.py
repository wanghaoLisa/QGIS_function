...
def statistics_by_type_intersection(point_layer, group_by_attribute, polygon_layer, polygon_attribute):
    ...
    for b in poly.getFeatures():
        ...
        s=set(list)#get the unique attribute
        s.remove('else')
        dic[b.id()]=len(s)# assign the number of unique attributes within each polygon
    d=collections.OrderedDict(sorted(dic.items()))#number of polygons
    _sum=0
    for k, v in d.items():
        _sum+=v# sum the total number of attributes in all of the polygons
#        print(k,":", v)
    print('mean:',_sum*1.000/len(d))# calcuate the average number of unique attributes in one polygon